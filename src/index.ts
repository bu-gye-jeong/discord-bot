...
const commands: Discord.Collection<string[], ICommand> =
...
const init = async () => {
  ...
  const eventFiles = (await readdir(path.join(__dirname, "/events"))).filter(
    (f) => f.endsWith(dev ? ".ts" : ".js")
  );
  ...
  const cmdFiles = (await readdir(path.join(__dirname, "/commands"))).filter(
    (f) => f.endsWith(dev ? ".ts" : ".js")
  );
  ...
  const gameFiles = (await readdir(path.join(__dirname, "/games"))).filter(
    (f) => f.endsWith(dev ? ".ts" : ".js")
  );
