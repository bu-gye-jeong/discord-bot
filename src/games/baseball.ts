import { IGame, prisma } from "../index";

export default <IGame>{
  name: ["야구", "숫자야구"],
  execute: async function (_client, msg, args) {
    const id = msg.author.id;
    let user = await prisma.baseball.findFirst({ where: { id: id } });
    const isNumber = (n: string) => n.match(new RegExp(/^[0-9]$/g));

    if (args[0] == "시작") {
      if (!user) {
        user = await prisma.baseball.create({
          data: { id: id, isStarted: false },
        });
      }
      if (!user.isStarted) {
        user.isStarted = true;
        ...
        await prisma.baseball.update({ where: { id: id }, data: user });
      ...
      if (!user) {
        ...
        msg.channel.send(
          user.best ? "최고 기록 : " + user.best : "기록이 없습니다!"
        );
      }
